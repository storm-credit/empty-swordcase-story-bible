#!/usr/bin/env python3
"""Validate completed v3.4 canon with blueprint-based supporting assignments."""
...
def load_episodes() -> dict[int, dict[str, Any]]:
...
def source_contains_name(row: dict[str, Any], name: str) -> bool:
...
def blueprint_support_names(blueprint: dict[str, Any]) -> set[str]:
    return {row.get("name") for row in blueprint.get("supporting_cast_additions", [])}
...
def main() -> int:
    ...
    require(len(episodes) == 200, f"episode function map has {len(episodes)}/200")
    ...
    blueprint_by_episode = {row["episode"]: row for row in blueprint_rows}
    ...
    require(len(blueprint_rows) == 180, f"blueprints must be 180, found {len(blueprint_rows)}")
    require(sum(len(row.get("scene_beats", [])) for row in blueprint_rows) == 1080, "scene beats must total 1080")

    require([row.get("id") for row in item_rows] == [f"CI{i:03d}" for i in range(1, 121)], "item IDs invalid")
    require([row.get("id") for row in set_rows] == [f"SET{i:03d}" for i in range(1, 25)], "set IDs invalid")
    require([row.get("id") for row in beast_rows] == [f"BS{i:03d}" for i in range(1, 19)], "beast IDs invalid")
    require([row.get("id") for row in cast_rows] == [f"SC{i:03d}" for i in range(1, 29)], "supporting cast IDs invalid")
    require([row.get("name") for row in cast_rows] == [row.get("name") for row in source_voices], "supporting cast source mismatch")

    for row in cast_rows:
        appearances = row.get("canonical_appearance_episodes", [])
        require(bool(appearances), f"{row.get('id')} has no canonical or blueprint appearance")
        require(appearances == sorted(set(appearances)), f"{row.get('id')} appearances not sorted/unique")
        for episode in appearances:
            require(episode in episodes, f"{row.get('id')} invalid episode {episode}")
            grounded = source_contains_name(episodes[episode], row["name"])
            if episode in blueprint_by_episode:
                grounded = grounded or row["name"] in blueprint_support_names(blueprint_by_episode[episode])
            require(grounded, f"{row.get('id')} EP{episode:03d} appearance not grounded")
        for field in (
            "private_goal", "hidden_insecurity", "refusal_line", "personal_loss_from_damun_choice",
            "choice_without_damun", "relationship_pressure", "ending_state",
        ):
            require(isinstance(row.get(field), str) and len(row[field].strip()) >= 18, f"{row.get('id')} weak {field}")
    ...
    human_main_names = {"담운", "서린화", "곽무석", "소예란", "진여강", "백장"}
    require(not (item_names & beast_names), "beast duplicated in item registry")
    require(not (beast_names & cast_names), "supporting human/beast collision")
    require(not (beast_names & human_main_names), "main human/beast collision")
    require("두리" in beast_names, "Duri must remain the named beast companion")
    ...
    for row in blueprint_rows:
        ...
        require(row.get("status") == "canon_production_blueprint", f"EP{episode:03d} not canon")
        require(row.get("author_approval_required") is False, f"EP{episode:03d} still pending approval")
        ...
        for addition in row.get("supporting_cast_additions", []):
            require(addition.get("canon_effect") == "기존 goal·choice·reward·hook·cost 불변", f"EP{episode:03d} supporting addition changes canon")
        for addition in row.get("beast_context_additions", []):
            require(addition.get("canon_effect") == "기존 goal·choice·reward·hook·cost 불변", f"EP{episode:03d} beast addition changes canon")

    require(approval.get("approval_id") == APPROVAL_ID, "approval ID mismatch")
    require(approval.get("approved") is True, "approval is false")
    require(audit.get("status") == "passed", "semantic audit failed")
    require(audit.get("critical_errors") == [], "semantic audit has critical errors")
    require(audit.get("expected_named_beast_companion") == "두리", "Duri exception not documented")
    require(audit.get("protected_sources_modified") is False, "protected sources marked modified")

    project = manifest.get("project", {})
    completion = manifest.get("completion", {})
    require(project.get("version") == "3.4-final-design-bible", "manifest version mismatch")
    require(project.get("status") == "design_blueprint_world_bible_complete", "design not marked complete")
    for key in (
        ...
        require(completion.get(key) is True, f"manifest gate false: {key}")
    require(completion.get("prose_manuscript") is False, "must not claim prose manuscript completion")
    require((ROOT / PATHS["freeze_doc"]).exists(), "freeze document missing")
